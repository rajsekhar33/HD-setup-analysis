...
for filenumber in xrange(start,end):
   fileno=str(filenumber).rjust(4,'0')
   filename=filedir3+'Prsk'+str(fileno)+'.txt'
   fname = open(filename,'rt')
   data = np.loadtxt(filename, usecols=(0,1,2))
...
for filenumber in xrange(start,end):
   fileno=str(filenumber).rjust(4,'0')
   filename=filedir4+'Prsk'+str(fileno)+'.txt'
   fname = open(filename,'rt')
   data = np.loadtxt(filename, usecols=(0,1,2))
...
for filenumber in xrange(start,end):
   fileno=str(filenumber).rjust(4,'0')
   filename=filedir5+'Prsk'+str(fileno)+'.txt'
   fname = open(filename,'rt')
   data = np.loadtxt(filename, usecols=(0,1,2))
...
plt.title(r'Compensated $P(k)$ vs k for $512^3$, different $k_{driving}$' )
plt.savefig('Prs_k_compensated_diff_k_512.png',dpi=250)
...
#ax.plot(k1[1:-10],k1[1:-10]**(-7./3.),'-',label='$k^{-7/3}$')
ax.set_yscale('log')
ax.set_xscale('log')
plt.xlabel('k')
plt.ylabel(r'$P(k)$')
...
# Put a legend to the right of the current axis
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.title(r'$P(k)$ vs k for $512^3$, different $k_{driving}$' )
plt.savefig('Prs_k_diff_k_512.png',dpi=250)

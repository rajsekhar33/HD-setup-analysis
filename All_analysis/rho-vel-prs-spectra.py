...
#Calculate standard deviation and mean
	...
	if (i==0):
                ...
                leg2=r'$\frac{|\rho_k|^2}{\left<\rho\right>^2}$'
                leg3=r'$\frac{|P_k|^2}{\left<P\right>^2}$'
	#Plot original power spectra
	#first velocity spectra
		spectra[0]=ax1.errorbar(k1[1:-10],Ek1[1:-10]/cs[i]**2., yerr=del_Ek1[1:-10]/cs[i]**2., color=colors[i], fmt='o', markeredgecolor=None, ecolor=None, capsize=None, barsabove=False, label=leg1, markersize=6., elinewidth=0.8)
		spectra[1]=ax1.errorbar(k1[1:-10],Ek2[1:-10], yerr=del_Ek2[1:-10], color=colors[i], fmt='v', markeredgecolor=None, ecolor=None, capsize=None, barsabove=False, label=leg2, markersize=6., elinewidth=0.8)
		spectra[2]=ax1.errorbar(k1[1:-10],Ek3[1:-10], yerr=del_Ek3[1:-10], color=colors[i], fmt='s', markeredgecolor=None, ecolor=None, capsize=None, barsabove=False, label=leg3, markersize=6., elinewidth=0.8)
	elif (i<num_plots):
		...
		ax1.errorbar(k1[1:-10],Ek3[1:-10], yerr=del_Ek3[1:-10], color=colors[i], fmt='s', markeredgecolor=None, ecolor=None, capsize=None, barsabove=False, markersize=6., elinewidth=0.8)
...
fit, =ax1.plot(x, 5e-2*x**(-5./3.), label=r'$k^{-5/3}$', marker="d", markeredgecolor='none', markersize=0.5, linewidth=2.0, color=colors[9])
ax1.set_yscale('log')
ax1.set_xscale('log')
...
spectra_legend1=ax1.legend(handles=spectra[1::], loc='lower left', bbox_to_anchor=(-0.05, -0.08), ncol=2, fancybox=True, framealpha=0., fontsize=26.)
ax1.add_artist(spectra_legend1)
spectra_legend2=ax1.legend(handles=spectra[0::3], loc='lower left', bbox_to_anchor=(-0.05, 0.1), fancybox=True, framealpha=0., fontsize=26.)
ax1.add_artist(spectra_legend2)
...
ax2.legend(loc='lower right',  ncol=3, fancybox=True, bbox_to_anchor=(1.02, -0.02), frameon=True, framealpha=0.8, fontsize=20)
...
plt.savefig('ratio-rho-vel-prs-spectra.png',dpi=200)

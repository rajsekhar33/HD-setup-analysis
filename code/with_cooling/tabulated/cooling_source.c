 ...
 *********************************************************************** */
{
  ...
  double *dx1, *dx2, *dx3;
  double dvol, vol;
  double sendarray[2], recvarray[2];//For parallel communication

  dx1 = GXYZ[IDIR].dx; dx2 = GXYZ[JDIR].dx; dx3 = GXYZ[KDIR].dx;
...
/*  ----------------------------------------------------------- 
  ...
  g_tot_cool=0.; //User edit: Initialise net cooling to zero, update it during the function.
  vol=0.;//User edit: Initialise total volume to zero, update it during the function.
  ...
  /* --------------------------------------------------
      ...
      (only for AMR)
     -------------------------------------------------- */ 
    dvol=dx1[i]*dx2[j]*dx3[k]; //Update volume of cell
...
/*  SolveODE_ROS34 (v0, k1, v1, dtsub, min_tol);  */
  ...
  /* ---- Update solution array ---- */
    g_tot_cool += (d->Vc[PRS][k][j][i]-prs)/(g_gamma-1.)/dt*dvol;//User edit: Updated total cooling rate
    vol+=dvol;
    d->Vc[PRS][k][j][i] = prs;

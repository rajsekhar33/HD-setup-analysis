 ...
 *********************************************************************** */
{
  ...
  double *dx1, *dx2, *dx3;
  double ***Fx1, ***Fx2, ***Fx3;
  double sendarray[12], recvarray[12], dvol;
  FILE *hist_file;
  ...
  if (g_stepNumber==0) {
    ...
    fprintf(hist_file,"#time g_dt mass TE KE1 KE2 KE3 MOM1 MOM2 MOM3 Net_cool Net_heating Mach del_rho/rho\n ");
  }
...
//    dvol = grid[IDIR].dV*grid[JDIR].dV*grid[KDIR].dV;
   ...
   sendarray[8]=g_v_rms; sendarray[9]=g_vol; sendarray[10]=g_rho_rms; sendarray[11]=g_cs; 
   MPI_Reduce (sendarray, recvarray, 12, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
   if (prank == 0){
     ...
     g_v_rms=recvarray[8]; g_vol=recvarray[9]; g_rho_rms=recvarray[10]; g_cs=recvarray[11];
  ...
  g_epsilon=g_tot_heat/g_mass;//Divide by mass to get the value of epsilon, the
                             //energy input rate per unit mass, 
			     //here mass is unity, so we needn't print epsilon explicitly
  fprintf(hist_file,"%20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e %20.10e\n ", g_time, g_dt, g_mass, g_TE, g_KE1, g_KE2, g_KE3, g_mom1, g_mom2, g_mom3, g_tot_cool, g_tot_heat, g_v_rms/g_cs, g_rho_rms/g_rho);
  fclose(hist_file);
  #ifdef PARALLEL
  }
  #endif
...
// apply turbulent forcing; only driving large scale modes. Follow Eswaran & Pope 1987
 ...
 if(g_stepNumber>1){
  ...
  DOM_LOOP(k,j,i){
   ...
   momx1 += dt*dvol*Fx1[k][j][i];
   momx2 += dt*dvol*Fx2[k][j][i];
   momx3 += dt*dvol*Fx3[k][j][i];
   mass += dmass;
  }
    #ifdef PARALLEL

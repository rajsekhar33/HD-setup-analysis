...
filedir='/mnt/lustre/ug4/ugrajs/with_viscosity/256/1e3/'
...
filedir='/mnt/lustre/ug4/ugrajs/with_viscosity/512/1e3/'
file=filedir+'pluto_hst.out'
fname = open(file,'rt')
...
ax.errorbar(k1[1:-10],Ek_comp1[1:-10],yerr=del_Ek_comp1[1:-10],fmt='*-',label=r'$\nu=10^{-3}$, $256^3$')
ax.errorbar(k2[1:-10],Ek_comp2[1:-10],yerr=del_Ek_comp2[1:-10],fmt='d-',label=r'$\nu=10^{-3}$, $512^3$')
ax.set_yscale('log')
ax.set_xscale('log')
plt.xlabel('k')
...
plt.title(r'Compensated E(k) vs k for $\nu=10^{-3}$, different resolution' )
plt.savefig('E_k_compensated_visc_resolution.png',dpi=150)

#This is to plot the original power spectrum, without any compensation

fig, ax = plt.subplots()
fig.set_size_inches(6, 5)
ax.errorbar(k1[1:-10],Ek1[1:-10],yerr=del_Ek1[1:-10],fmt='*-',label=r'$\nu=10^{-3}$, $256^3$')
ax.errorbar(k2[1:-10],Ek2[1:-10],yerr=del_Ek2[1:-10],fmt='d-',label=r'$\nu=10^{-3}$, $512^3$')
...
# Put a legend to the right of the current axis
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.title(r'E(k) vs k for $\nu=10^{-3}$, different resolution' )
plt.savefig('E_k_visc_resolution.png',dpi=150)

...
wdir=('T-runs/Tl/', 'T-runs/Th/', 'B-runs/BDh2/')
labels=('Tl', 'Th', 'BDh2')

step_size=0.2
#end=np.array((25., 53.2, 22.2, 53.2))#, 55.4, 31.))
end=np.array((25., 53.2, 33.6))
start=np.ones(end.size)*step_size
start_time=np.array((10., 51., 10.))
#Load data files
...
#       fig, ax = plt.subplots(1)
	...
	print file 
        #Plot the data
	if(i1!=2):
		ax.plot(data[:,2][:int(start_time[i1]/step_size)],5./3.*data[:,1][:int(start_time[i1]/step_size)], label=labels[i1], color=colors[i1], marker=".", markersize=3., linewidth=.5)
		perturb2[0], =ax.plot(data[:,2][int(start_time[i1]/step_size):],5./3.*data[:,1][int(start_time[i1]/step_size):], label=r'$\frac{5}{3}\frac{\left<\delta\rho\right>_{rms}}{\left<\rho\right>}$', color=colors[i1], marker=".", markersize=6., linewidth=0.5)
		perturb1[i1],=ax.plot(data[:,2][:int(start_time[i1]/step_size)],data[:,0][:int(start_time[i1]/step_size)], label=labels[i1], color=colors[i1], marker="v", markersize=3., linewidth=.5)
		perturb2[1], =ax.plot(data[:,2][int(start_time[i1]/step_size):],data[:,0][int(start_time[i1]/step_size):], label=r'$\frac{\left<\delta P\right>_{rms}}{\left< P\right>}$', color=colors[i1], marker="v", markersize=6., linewidth=0.5)
	else:
		ax.plot(data[:,2][:int(start_time[i1]/step_size)],5./3.*data[:,1][:int(start_time[i1]/step_size)], label=labels[i1], color=colors[i1+5], marker=".", markersize=3., linewidth=.5)
		perturb2[0], =ax.plot(data[:,2][int(start_time[i1]/step_size):],5./3.*data[:,1][int(start_time[i1]/step_size):], label=r'$\frac{5}{3}\frac{\left<\delta\rho\right>_{rms}}{\left<\rho\right>}$', color=colors[i1+5], marker=".", markersize=6., linewidth=0.5)
		perturb1[i1],=ax.plot(data[:,2][:int(start_time[i1]/step_size)],data[:,0][:int(start_time[i1]/step_size)], label=labels[i1], color=colors[i1+5], marker="v", markersize=3., linewidth=.5)
		perturb2[1], =ax.plot(data[:,2][int(start_time[i1]/step_size):],data[:,0][int(start_time[i1]/step_size):], label=r'$\frac{\left<\delta P\right>_{rms}}{\left< P\right>}$', color=colors[i1+5], marker="v", markersize=6., linewidth=0.5)

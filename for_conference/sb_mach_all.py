...
ax.plot(y,0.25*y**2,dashes=[2, 2], color=colors[6], linewidth=2.)
...
#       fig, ax = plt.subplots(1)
	...
	if(i1!=2):
		...
		perturb2[i1], =ax.plot(data[:,2][int(start_time[i1]/step_size):del_R.size], del_R[int(start_time[i1]/step_size):], label=labels[i1], color=colors[i1], marker=".", markeredgecolor='none', markersize=8., linewidth=1.)
	else:
		perturb1[i1], =ax.plot(data[:,2][1:int(start_time[i1]/step_size)], del_R[1:int(start_time[i1]/step_size)], label=labels[i1], color=colors[i1+5], marker=".", markeredgecolor='none', markersize=4., linewidth=1.)
		perturb2[i1], =ax.plot(data[:,2][int(start_time[i1]/step_size):del_R.size], del_R[int(start_time[i1]/step_size):], label=labels[i1], color=colors[i1+5], marker=".", markeredgecolor='none', markersize=8., linewidth=1.)

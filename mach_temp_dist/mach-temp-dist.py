...
for i in xrange(num_plots):
	...
	data = np.loadtxt(file, skiprows=1, usecols=(0,1))
	distM[j+num_plots], =ax1.plot(data[:,0], data[:,1]/(float(nx*ny*nz)), dashes=[30, 5, 10, 5], label=labels[i]+', t='+str(int(t_mp[i]*UNIT_TIME))+' Myr')
	...
	data = np.loadtxt(file, skiprows=1, usecols=(0,1))
	distT[j], =ax2.plot(data[:,0], data[:,1]/(float(nx*ny*nz)), label=labels[i]+', t='+str(int(t_turb[i]*UNIT_TIME))+' Myr')
	file=filedir+'temp'+str(int(t_mp[i]/step_size+0.1)).rjust(4,'0')+'.txt'
	data = np.loadtxt(file, skiprows=1, usecols=(0,1))
	distT[j+num_plots], =ax2.plot(data[:,0], data[:,1]/(float(nx*ny*nz)), dashes=[30, 5, 10, 5], label=labels[i]+', t='+str(int(t_mp[i]*UNIT_TIME))+' Myr')
	ax2.set_xscale('log')
	ax2.set_yscale('log')
	ax2.set_xlim(1e5, 1e8)
	ax2.set_ylim(1e-4, 2e-1)
	ax2.set_xlabel(r'$T$')
	ax2.set_ylabel(r'Volume fraction')
	box = ax2.get_position()
	ax2.set_position([box.x0, box.y0, box.width, box.height])
	# Put a legend to the bottom of the current axis
	ax2.legend(loc='upper left', ncol=2, fancybox=True, framealpha=0.)
...
distM_legend=ax1.legend(handles=distM, loc='upper left', ncol=2, fancybox=True, framealpha=0.)

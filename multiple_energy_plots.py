...
file3='/home/rajsekhar/PLUTO41_old/3D_turb/Tau_c_2/with_viscosity/512/4/Ek0003.txt'
...
ax.plot(k3,Ek_comp3,'d-',label=r'$\nu = $'+str(nu3))
ax.set_yscale('log')
ax.set_xscale('log')
plt.xlabel('k')
plt.ylabel('E(k)*$k^{5/3}$')
leg = ax.legend(loc=2, bbox_to_anchor=(0.75, 1.15))
plt.title('Compensated E(k) vs k at t='+str(float(filenumber))+' size '+str(n[0])+'$^3$' )
...
plt.title('E(k) vs k at t='+str(float(filenumber))+' size '+str(n[0])+'$^3$')

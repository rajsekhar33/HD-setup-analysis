...
"""	plt.title(r'$\frac{\delta\rho}{\rho}$ vs $\mathcal{M}$ for amp='+str(amp[i1]))
...
#ax.plot(x,0.35*x**2,label='$\mathcal{M}^2$')
ax.plot(y,0.4*y-0.1,label='$\mathcal{M}$')
ax.set_yscale('log')
ax.set_xscale('log')
plt.title(r'$\frac{\delta\rho}{\rho}$ vs $\mathcal{M}$')

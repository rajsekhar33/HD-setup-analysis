void read_dbl(int f, double ****vel)
{
   ...
   for(dir=0;dir<nv;dir++){
     for(i=0;i<nx;i++){
       for(j=0;j<ny;j++){
         for(k=0;k<nz;k++){
           offset = i*ny*nz+j*ny+k + 4*dir*nx*ny*nz;//Since we are scanning only rho and pressure, for pressure, offset is 4*nx*ny*nz
...
void write_file_Ek_binned(int f, int dir, Ek *E_k_binned, Ek *E_k_comp)
{
   ...
   sprintf(filenumb,"%04d",f);
   strcpy(filename,datdir);
   if(dir==1) strcat(filename,outprefix);
   else if(dir==0) strcat(filename,"delrhok");
   strcat(filename,filenumb);
   strcat(filename,".txt");
   printf("%s\n",filename);
   int i;
...
void write_file_Ek(int f, int dir, Ek *E_k_added)
{
   FILE *fp;
   double d;
   char filenumb[5];
   ...
   sprintf(filenumb,"%04d",f);
   strcpy(filename,datdir);
   if(dir==1) strcat(filename,"delP_k_added");
   else if(dir==0) strcat(filename,"delrho_k_added");
   strcat(filename,filenumb);
   strcat(filename,".txt");
   printf("%s\n",filename);
   int i;

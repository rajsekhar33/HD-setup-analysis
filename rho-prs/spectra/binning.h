...
void scalar_counter(Ek *E_k, Ek *E_k_added){
...
void bin(Ek *E_k_added, Ek *E_k_binned){
  ...
  r_bin1=powl(max_k/ratio,(double)1.0/(no_bins));
...
void calc_comp(Ek *E_k_comp, Ek *E_k_binned, int dir){
  int tot_bins=no_bins;
  int i;
  for(i=0;i<tot_bins;i++){
    E_k_comp[i].k_sq=E_k_binned[i].k_sq;
    if (dir==0) E_k_comp[i].energy=E_k_binned[i].energy * pow(E_k_binned[i].k_sq,5.0/3.0);
    else if (dir==1) E_k_comp[i].energy=E_k_binned[i].energy * pow(E_k_binned[i].k_sq,7.0/3.0);
  }
}

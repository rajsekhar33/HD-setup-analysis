...
wdir=('T-runs/Tl/', 'T-runs/Th/', 'T-runs/Tlr/', 'T-runs/Thr/', 'B-runs/BDh2/')
#labels=('Tl', 'Th', 'Bl', 'Bh', 'QD', 'TDh', 'BDh')
#labels=('Tl', 'Th', 'Bl', 'Bh', 'TDh', 'BDh')
labels=('Tl', 'Th', 'Tlr', 'Thr', 'BDh2')
...
"""
step_size=0.2
end=np.array((24.8, 53.2, 16.8, 39.4, 31.))
start_time=np.array((10., 51., 16., 36., 10.))
...
#       fig, ax = plt.subplots(1)
	...
	j1=i1
	if(i1==4):j1=i1+3
	sb_vlos[i1],=ax.plot(x1, dist_data/height, color=colors[j1], marker=".", markeredgecolor='none', markersize=0.1, linewidth=2., label=labels[i1]+', $\sigma_v=$'+str(int(popt[1]))+'km/s')
	print popt[1]
        #Plot the data
...
ax.set_xlim(-1e3, 1e3)
ax.set_ylim(0., 3e-3)
plt.savefig('sb-vlos-pdf-hr.png',dpi=250)
ax.set_ylim(1.e-5, 1e-2)
ax.set_yscale('log')
plt.savefig('sb-vlos-pdf-log-hr.png',dpi=250)

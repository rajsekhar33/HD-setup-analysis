...
#       fig, ax = plt.subplots(1)
	...
	for j in xrange(int(num_snap[i1])):
		...
		data[:,1]=data[:,1]/(np.sum(data[:,1])*no_bins/2.e1)
		dist_data[j]=data[:,1]
	del_dist_data=np.std(dist_data,0)
	dist_data=np.average(dist_data,0)
        #Load data
        #Plot the data
        sb_vlos[i1] =ax.errorbar(UNIT_VELOCITY*data[:,0], dist_data, yerr=del_dist_data, label=labels[i1], color=colors[i1], fmt=".", markeredgecolor='none', markersize=10.0, linewidth=1.)
sb_vlos[i1+1],=ax.plot(x, y, label='Hitomi', color=colors[i1+1], marker=".", markeredgecolor='none', markersize=0.1, linewidth=2.)

sb_legend=ax.legend(handles=sb_vlos, loc='upper right', bbox_to_anchor=(1.05, 1.07), ncol=4, fontsize=28.)
ax.add_artist(sb_legend)
sb_legend.get_frame().set_alpha(0.)
